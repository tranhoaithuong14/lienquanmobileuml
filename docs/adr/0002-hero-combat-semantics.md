# 0002 — Hero combat semantics (float HP, final hp, heal no-op when dead, separate respawn)
...
2. **`hp` final, set at constructor** — simple, sufficient for current scope. Level-up / buffs that change `hp` come later (will need a dedicated `setHp` method). The label `hp` (not `maxHp`) matches the in-game hero detail screen.
...
4. **`respawn()` is a separate method** — matches gameplay: triggered by an external respawn timer, not auto-revived from `heal`. Sets `alive=true`, `currentHp=hp`.
...
- **`hp` mutable with `setHp` setter**: needs a test for "currentHp clamps to the new hp when hp shrinks". Too anticipatory for current scope.
...
- `Hero` constructor throws `IllegalArgumentException` when `hp <= 0`. Tests for the exception path were not initially written — coverage gap closed in subsequent test hygiene work.
