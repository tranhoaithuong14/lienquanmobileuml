...
- `src/main/java/com/moba/combat/BaseStats.java` — `record` Value Object composed of `CoreStats` + `OffensiveStats` + `AttackRange`, immutable. Compact constructor validates caps verified in ADR-0003 R7 (critDamage ≥ 1.0, cooldownReduction ∈ [0, 0.40], critChance ∈ [0, 1], movementSpeed > 0, maxHp > 0, attackRange non-null).
...
- `BaseStatsTest` (8): equality, validation for maxHp, movementSpeed, critChance, critDamage, cooldownReduction, attackRange, plus an explicit "high critDamage allowed" test.
...
3. **Premature generalization still wins by losing less** — ADR-0003 R2 recommended starting with a flat record, deferring `CoreStats`/`OffensiveStats`/`Vitals` split. We did; 17 fields in one record is readable because of the section comments and the cluster of related fields (pen pairs together).
